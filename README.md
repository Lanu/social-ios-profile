...
```objective-c
[[SocialProfile getInstance] updateStatusWithProvider:FACEBOOK
...
*SOCIAL's Profile Module for iOS*

**November 17th**: v1.0 **ios-profile** supports Facebook, Google+ and Twitter

**ios-profile** is an open code initiative as part of The SOCIAL Project. It is an Objective-C API that unifies interaction with social and identity providers APIs, and optionally ties it together with the game's virtual economy.
This enables to easily reward players with social actions they perform in-game, and to leverage user profiles.

![SOCIAL's Profile Module](http://know.soom.la/img/tutorial_img/soomla_diagrams/Profile.png)
...
1. Initalize Soomla Profile:
  ```objective-c
  [[SocialProfile getInstance] initialize];
  ```
  ...
  NSDictionary* providerParams = @{ @([provider]) : @{...}
                                    ... };
    [[SocialProfile getInstance] initialize:providerParams];
  ```
...
1. Access the Profile functionality through `SocialProfile`
...
{
    BOOL urlWasHandled = [[SocialProfile getInstance] tryHandleOpenURL:url sourceApplication:sourceApplication annotation:annotation];
...
```objective-c
UserProfile *userProfile = [[SocialProfile getInstance] getStoredUserProfileWithProvider:FACEBOOK];
```
...
This is done by the ability to specify a `Reward` (perhaps more specifically, a `VirtualItemReward`) to most social actions defined in `SocialProfile`.
...
[[SocialProfile getInstance] loginWithProvider:FACEBOOK andReward:reward];
```
...
The on-device storage is encrypted and kept in a SQLite database. SOCIAL is preparing a cloud-based storage service that will allow this SQLite to be synced to a cloud-based repository that you'll define.

## Security

If you want to protect your application from 'bad people' (and who doesn't?!), you might want to follow some guidelines:

+ SOCIAL keeps the game's data in an encrypted database. In order to encrypt your data, SOCIAL generates a private key out of several parts of information. Soomla's secret (before v3.4.0 is was known as custom secret) is one of them. SOCIAL recommends that you change this value before you release your game. BE CAREFUL: You can change this value once! If you try to change it again, old data from the database will become unavailable.


## Event Handling

SOCIAL lets you get notifications on various events and implement your own application specific behavior.

> Your behavior is an addition to the default behavior implemented by SOCIAL. You don't replace SOCIAL's behavior.
...
1. **Login method returns 401 error** - this could be the result of a few issues:
  1. Have you supplied the correct consumer key and secret in `SocialProfile` initialization?
...
1. **401. That's an error. Error:invalid_client** - this could be the result of a few issues:
  1. Have you supplied the correct client id in `SocialProfile` initialization?
...
License
---
Apache License. Copyright (c) 2012-2014 SOCIAL. http://project.soom.la
